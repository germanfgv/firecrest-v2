...
def is_system_avail_f7t(system_name: str) -> bool:
    ...
    try:
        list_of_systems = f7t_client.systems()

        system = next((system for system in list_of_systems if
                       system["name"] == system_name), None)

        if system is None:
            app.logger.error(f"System {system_name} not found")
            return False

        return True

    except f7t.FirecrestException as fe:
        app.logger.error(f"Error getting system: {fe} ")
        return False
...
def get_username_with_f7t(system_name):
    ...
    try:
        ...
        userinfo = f7t_client.userinfo(system_name)
        return userinfo["user"]["name"]
    except f7t.FirecrestException as fe:
        app.logger.error(f"Error getting username: {fe} ")
        return None
...
def get_base_fs_with_f7t(system_name: str) -> str:
    ...
    try:
        ...
        filesystem = next((filesystem for filesystem in system["fileSystems"] if
                           filesystem["defaultWorkDir"]), None)
...
def submit_job_with_f7t(system_name: str, working_dir: str, job_script: str) -> dict:
    ...
    try:
        job = f7t_client.submit(system_name, working_dir, script_local_path=job_script)
...
def mkdir_with_f7t(system_name: str, target_path: str) -> bool:
    ...
    try:
        f7t_client.mkdir(system_name, target_path, create_parents=True)
        return True
    except f7t.FirecrestException as fe:
...
def list_jobs_with_f7t(system_name: str, job_ids: list[int]) -> dict:
    ...
    try:
        ...
        for job_id in job_ids:
            job_info = f7t_client.job_info(system_name, job_id)[0]
            
            if job_info["time"]["start"] is None:
                st_object = datetime.datetime.fromtimestamp(0)
            else:
                st_object = datetime.datetime.fromtimestamp(int(job_info["time"]["start"]))

            
            job_state = job_info["status"]["state"]
            if job_info["status"]["state"][0:9] == "CANCELLED":
                job_state = "CANCELLED"
            
            
            start_time = st_object.strftime('%Y-%m-%dT%H:%M:%S')
