...
Tha maximum size of the parts can be configured via the [`max_part_size parameter`](#config_page_bucket_lifecycle_configuration). The specified value must be lower than [S3's maximum limit](https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html) of 5GB per part and it is applied to any upload data call.
